...
exports.BaseRepository = class BaseRepository {
  ...
  getLiteralFrom(model, uuid, key = "uuid") {
    return this.sequelize.literal(`(SELECT id FROM "${model}" WHERE ${key} = '${uuid}')`);
  }
  ...
  customSorting(modelName, orderColumn, orderType, orderColumnType) {
    ...
    if (orderColumnType === DataTypes.STRING.key || orderColumnType === DataTypes.TEXT.key) {
      return [[this.sequelize.fn('lower', this.sequelize.col(`${modelName}.${orderColumn}`)), orderType]];
    } else {
   ...
   */
  async findOne(criteria, include = [], paranoid = true, attributes, transaction, options) {
   ...
   * @param {object} attributes - To exclude or include column in records.
   */
  async findAll(criteria, include = [], paranoid = true, attributes, transaction,options={}) {
 ...
 * @param {boolean} paranoid - If you really want to let the query see the soft-deleted records, you can pass the paranoid: false option to the query method
 */
async sum(criteria, field, include = [], paranoid = true, transaction, options = {}) {
   ...
   * @param {object} attributes - To exclude or include column in records.
   */
  async findAndCountAll(criteria, include = [], offset = 0, limit = 10, order, paranoid = true, attributes, transaction, options= {}) {
  ...
  async destroy(criteria, force = false, include, transaction, returning = ["*"]) {
    return this.model.destroy({ where: criteria, force, include, transaction, returning });
  }
