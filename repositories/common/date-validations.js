...
const { AccuralPeriod } = require("../../models/leave/accural-period-enum");
...
exports.isAccrualPeriodEnded = (joinDate, accrualPeriod, accrualApplicableOn) => {
    ...
    const calculateStartOfPeriod = (joinDateObj, accrualPeriod, accrualApplicableOn) => {
        ...
        switch (accrualPeriod) {
            case AccuralPeriod.ENUM.MONTHLY:
                if (accrualApplicableOn === AccuralApplicableOn.ENUM.DATE_OF_JOINING) {
                    return new Date(year, month, joinDateObj.getDate());
                } else if (accrualApplicableOn === AccuralApplicableOn.ENUM.START) {
                    return new Date(year, month, 1);
                } else if (accrualApplicableOn === AccuralApplicableOn.ENUM.END) {
                    return new Date(year, month + 1, 0);
                }
                break;
            case AccuralPeriod.ENUM.QUARTERLY:
                ...
                break;
            case AccuralPeriod.ENUM.HALF_YEARLY:
    ...
    switch (accrualPeriod) {
        ...
        case AccuralPeriod.ENUM.HALF_YEARLY:
            ...
            break;
        case AccuralPeriod.ENUM.YEARLY:
